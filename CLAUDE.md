...
If you catch yourself about to write TypeScript, JSX, CSS, Prisma schema, or
shell scripts for the app — **stop**. Write a spec instead and let Codex implement it.
...
- Spec must include: **Overview, User Stories, API Changes, Data Model Changes,
  UI/UX Notes, Acceptance Criteria, Out of Scope**.
- Update `docs/spec/README.md` index and `docs/openapi.yaml` if API changes.
...
## UI/UX Notes
Screens affected, component changes.
...
- **OpenAPI:** `docs/openapi.yaml` — must stay in sync with API changes
- **Stack:** Next.js 15, TypeScript 5, Tailwind CSS 4, shadcn/ui, Prisma 7, PostgreSQL
- **Auth:** JWT via `lib/auth.ts`; web=httpOnly cookie, extension=Bearer token
- **AI:** Vercel AI SDK + OpenAI (card gen) + Deepseek Vision (OCR)
